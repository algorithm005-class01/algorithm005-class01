...
class Solution:
    ...
    def myAtoi_1(self, str: str) -> int:
        s = str.strip()
        if (not s) or (not (s[0].isdecimal() or s[0] in ['+', '-'])):
            return 0
    ...
    def myAtoi(self, str: str) -> int:
        ...
        ans = 0 if not res or res[0] in ['+', '-'] else int(res[0])
        if ans > (1 << 31) - 1:
            ans = (1 << 31) - 1
        if ans < -1 << 31:
            ans = -1 << 31

        # res 为空列表或单元素时，int(*res) 才成立
        # ans = max(min(int(*res), (1<<31) - 1), -1<<31)
        return ans
